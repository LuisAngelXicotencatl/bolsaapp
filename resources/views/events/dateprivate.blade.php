...
<main>
    <div class="p-6 max-w-4xl mx-auto bg-white rounded-lg shadow-md">
            ...
            @foreach($EventDates as $eventDate)
                <div class="p-6 rounded-lg shadow-md border border-gray-300">Cita {{ $eventDate->Dateprivate->id }}
                    ...
                    <div class="mt-6 space-x-4">
                        <form action="{{route('eventprivate.destroyDateprivate', $eventDate->id)}}" method="POST">
                            @csrf
                            @method('delete')
        ...
        <div class="mt-6 space-y-4">
            <form action="{{ route('Eventprivate.newdateprivateprocess', $event->id) }}" method="post" class="space-y-4">
                @csrf
                @method('put')
